...
def hopkins_test(X: pd.DataFrame):
    ...
    for j in range(0, m):
        u_dist, _ = nbrs.kneighbors(
            np.random.uniform(np.amin(X, axis=0), np.amax(X, axis=0), d).reshape(1, -1),
            2,
...
def plot_scatter_with_centroids(x: str, y: str, scaler, model, df):
    ...
    sns.scatterplot(data=df, x=x, y=y, hue="segment", ax=ax)
    ...
    try:
        ...
        sns.scatterplot(data=centroids, x=x, y=y, color="black", marker="x", s=1000, ax=ax)
    ...
    sns.move_legend(ax, "upper center", title=None, ncol=5, bbox_to_anchor=(0.5, 1.05))
...
def plot_bar_with_hue(hue: str, df: pd.DataFrame):
    ...
    ax.spines[["right", "top"]].set_visible(False)
    sns.move_legend(
        ax, loc="upper center", bbox_to_anchor=(0.5, 1.14), ncols=5, title=None
    )
...
def plot_kde_box(x: str, y: str, df):
    ...
    sns.kdeplot(data=df, x=x, hue=y, fill=True, ax=ax[0])
    sns.boxplot(data=df, x=x, y=y, saturation=0.8, ax=ax[1])
    plt.suptitle(
        f'{x.title().replace("_", " ")} by {y.title()}', fontweight="bold", y=0.95
    )
    ...
    sns.move_legend(
        ax[0], "upper center", title=y.title(), ncol=5, bbox_to_anchor=(0.5, 1.1)
    )
    for i in (0, 1):
